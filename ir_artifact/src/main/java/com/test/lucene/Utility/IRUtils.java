package com.test.lucene.Utility;

import com.test.lucene.QueryResult;
...
public class IRUtils {
    ...
    public final static String absPathstop = String.format("%s/Docs/stopWords.txt", currentRelativePath);
    ...
    public static ArrayList<Document> loadFedRegisterDocs(String pathToFedRegister, String id, String setitle,
        ...
        if (pathToFedRegister.equals(absPathFB) || pathToFedRegister.equals(absPathLA)) {
        ...
        for (File directory : directories) {
            ...
            for (File file : files) {
                ...
                for (org.jsoup.nodes.Element document : documents) {
                    ...
                    if (pathToFedRegister.equals(absPathFT)) {
    ...
    public ArrayList<ArrayList<String>> getArrQueries() throws Exception {
        ...
        for (org.jsoup.nodes.Element document : documents) {
            ...
            String title=document.select("title").text().replaceAll("\\s+", " ");
            String num=document.select("num").text().replaceAll("\\s+", " ");
            String desc=document.select("desc").text().replaceAll("\\s+", " ");
            desc=desc.substring(desc.indexOf(":")+1, desc.indexOf("Narrative")).trim();
            String narr=document.select("narr").text().replaceAll("\\s+", " ").trim();
